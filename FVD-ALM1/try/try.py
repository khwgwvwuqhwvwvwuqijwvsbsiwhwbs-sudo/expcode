...
class TFEDCN(torch.nn.Module):
    ...
    def __init__(self, n_feature, n_class, actiondict=None, actiontoken=None, inp_actionlist=None, **args):
        ...
        self.vAttn = getattr(model, args['opt'].AWM)(1024, args)#Modality_Enhancement_Module
        self.fAttn = getattr(model, args['opt'].TCN)(1024, args)#TFE_DC_Module

        self.vAttn1 = Attn(1024, args['opt'].sig_T_attn)
        self.fAttn1 = Attn(1024, args['opt'].sig_T_attn)
        self.alpha=args['opt'].convex_alpha
    ...
    def forward(self, inputs, itr, split, is_training=True, **args):
        ...
        v_atn1, vfeat1, n_rfeat1, o_rfeat1 = self.vAttn1(feat[:, :1024, :], feat[:, 1024:, :])
        f_atn1, ffeat1, n_ffeat1, o_ffeat1 = self.fAttn1(feat[:, 1024:, :], feat[:, :1024, :])

        x_atn = self.alpha * f_atn1 + (1 - self.alpha) * v_atn1

        nfeat1 = torch.cat((vfeat1, ffeat1), 1)
        nfeat_out0 = self.fusion(nfeat1)
        ...
        return {'feat': nfeat.transpose(-1, -2), 'cas': x_cls.transpose(-1, -2), 'attn': x_atn.transpose(-1, -2),
                'v_atn': v_atn1.transpose(-1, -2), 'f_atn': f_atn1.transpose(-1, -2), 'mu': mu,
