...
class TFEDCN(torch.nn.Module):
    ...
    def forward(self, inputs, is_training=True, **args):
        ...
        # 2. 计算注意力权重：mu 作为 Query，h 作为 Key/Value
        ...
        # 4. 拼接或加和
        #print("proj_mu的维度：",proj_mu.shape)
        #print("h_context的维度：",h_context.shape)
        
        #print("=== DEBUG ===")
        #print("proj_mu actual shape:", proj_mu.shape)
        #print("h_context actual shape:", h_context.shape)
        #print("proj_mu device:", proj_mu.device)
        #print("h_context device:", h_context.device)
        #print("proj_mu dtype:", proj_mu.dtype)
        #print("h_context dtype:", h_context.dtype)
        ...
        fused = nn.Linear(1024, 2048).cuda()(fused)  # [10, 8, 2048]
        #print("fused的维度：",fused.shape)
        
        fused = self.EM(fused, nfeat.transpose(-1, -2))
        ...
        mu_cls = self.classifier(fused.transpose(-1, -2))
