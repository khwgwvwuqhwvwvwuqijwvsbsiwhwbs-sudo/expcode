...
class SampleDataset:
    def __init__(self, args, mode="both",sampling='random'):
        ...
        self.path_to_features = os.path.join(self.dataset_name,self.dataset_name + "-I3D-JOINTFeatures.npy")
...
#-------------------------------------------------------------------------
        self.clip_path = args.path_clip_dataset#新增
        ...
        with open('./Thumos14reduced/gt.json') as j:
    ...
    def load_data(self,n_pro=14, n_similar=0, is_training=True, similar_size=2):
        if is_training:
            ...
            for i in idx:
                ...
                # --- [新增] CLIP 特征加载和时序标注获取 (从 wsad_dataset.py 复制) ---
                # 加载预计算的 CLIP 特征文件 (.npy)
                clip_ifeat = np.load(os.path.join(self.clip_path, 'train', self.videonames[i].decode('utf-8') + '.npy'))

                # 获取时序标注 (需在采样前获取)
                cur_temp_anno = self.temp_label[i]
                # --------------------------------------------------------------------------
                ...
                # --- [新增] 应用采样并存储 CLIP 特征和时序标注 ---
                clip_ifeat = clip_ifeat[sample_idx]
                clip_feat.append(clip_ifeat)

                cur_temp_anno = cur_temp_anno[:, sample_idx]
            ...
            return feat, clip_feat, np.array(
                temp_anno), labels, rand_sampleid, words_batch, words_feat_batch, words_id_batch, words_weight_batch, words_len_batch
            ...
            # --- [新增] CLIP 特征加载 ---
            clip_feat = np.load(os.path.join(self.clip_path, 'test',
                                             self.videonames[self.testidx[self.currenttestidx]].decode(
                                                 'utf-8') + '.npy'))
            # ---------------------------
