...
def test(itr, dataset, args, model, logger, device, pool):
    ...
    while not done:
        ...
        with torch.no_grad():
            ...
            outputs = model(Variable(features), itr=itr, split='test')
            element_logits = outputs['cas']
            results[vn] = {'cas': outputs['cas'], 'attn': outputs['attn'],
                           'mu_v': outputs['mu_v'].detach().cpu().numpy(),
                           'emb_v': outputs['emb_v'].detach().cpu().numpy(),
                           'var_v': outputs['var_v'].detach().cpu().numpy(),
                           # clip_feat 是原始数据，用于外部对比
                           'clip_feat': clip_feat.squeeze(0).cpu().numpy(),
                           }
...
if __name__ == '__main__':
    ...
    # model 未定义，应该是 model1
    iou, dmap = test(-1, dataset, args, model1, logger, device, pool)
    # for jdx in range(9):
    #    print(dap[jdx])
    print('||'.join(['MAX map @ {} = {:.3f} '.format(iou[i], dmap[i] * 100) for i in range(len(iou))]))
    print('mAP Avg 0.1-0.5: {}, mAP Avg 0.3-0.7: {}, mAP Avg ALL: {}'.format(np.mean(dmap[:5]) * 100,
                                                                             np.mean(dmap[2:7]) * 100,
                                                                             np.mean(dmap) * 100))
